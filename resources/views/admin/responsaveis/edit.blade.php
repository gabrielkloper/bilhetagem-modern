...
<div class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Editando dados de "{{ $responsavel->nome }}" {{ $inscricaoAtual && $inscricaoAtual->evento ? 'no evento "' . $inscricaoAtual->evento->titulo . '"' : '' }}
            </p>
        </div>
        <div class="mt-4 sm:mt-0 flex space-x-3">
            <a href="{{ route('admin.responsaveis.show', ['responsavel' => $responsavel, 'evento_id' => $eventoId]) }}" 
    ...
    <div class="bg-white shadow rounded-lg">
        <form method="POST" action="{{ route('admin.responsaveis.update', $responsavel) }}" class="space-y-6 p-6" 
              data-confirm="Confirma as alterações realizadas nos dados do responsável?">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="cpf" id="cpf" required 
                               value="{{ old('cpf', $responsavel->cpf_formatted) }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="nascimento" id="nascimento" required 
                               value="{{ old('nascimento', $responsavel->nascimento->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="tel" name="telefone1" id="telefone1" required 
                               value="{{ old('telefone1', $responsavel->telefone1) }}"
                    ...
                    <div>
                        ...
                        <input type="tel" name="telefone2" id="telefone2" 
                               value="{{ old('telefone2', $responsavel->telefone2) }}"
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        <div class="flex items-center">
                            <input id="comunica" name="comunica" type="checkbox" 
                                   {{ old('comunica', $inscricaoAtual ? $inscricaoAtual->comunica : false) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <select name="device_comunica" id="device_comunica" required 
                            ...
                            <option value="email" {{ old('device_comunica', $inscricaoAtual ? $inscricaoAtual->device_comunica : 'email') == 'email' ? 'selected' : '' }}>E-mail</option>
                            <option value="sms" {{ old('device_comunica', $inscricaoAtual ? $inscricaoAtual->device_comunica : 'email') == 'sms' ? 'selected' : '' }}>SMS</option>
                            <option value="whatsapp" {{ old('device_comunica', $inscricaoAtual ? $inscricaoAtual->device_comunica : 'email') == 'whatsapp' ? 'selected' : '' }}>WhatsApp</option>
                            <option value="todos" {{ old('device_comunica', $inscricaoAtual ? $inscricaoAtual->device_comunica : 'email') == 'todos' ? 'selected' : '' }}>Todos os meios</option>
                        </select>
                ...
                <div class="mt-4">
                    <div class="flex items-center">
                        <input id="ativo" name="ativo" type="checkbox" 
                               {{ old('ativo', $inscricaoAtual ? $inscricaoAtual->ativo : true) ? 'checked' : '' }}
                               class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div id="vinculados-container">
                    <!-- Existing vinculados will be loaded here via JavaScript -->
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addVinculado(data = {}) {
        ...
        const vinculadoHtml = `
            <div class="vinculado-item border border-gray-200 rounded-lg p-4 mb-4" data-index="${vinculadoIndex}">
                ...
                ${data.id ? `<input type="hidden" name="vinculados[${vinculadoIndex}][id]" value="${data.id}">` : ''}
                
                <div class="mt-3">
                    <div class="flex items-center">
                        <input type="checkbox" name="vinculados[${vinculadoIndex}][_delete]" 
    ...
    window.removeVinculado = async function(button) {
        const result = await SweetAlert.confirmDelete('este vinculado');
        
        if (result.isConfirmed) {
            button.closest('.vinculado-item').remove();
            SweetAlert.toast('success', 'Vinculado removido');
        }
    };

    // Load existing vinculados
    @foreach($responsavel->vinculados as $vinculado)
        addVinculado({
            id: {{ $vinculado->id }},
            nome: "{{ $vinculado->nome }}",
            nascimento: "{{ $vinculado->nascimento->format('Y-m-d') }}",
            vinculo_id: {{ $vinculado->vinculo_id }},
            lembrar: {{ $vinculado->lembrar ? 'true' : 'false' }}
        });
    @endforeach
    ...
    @if(old('vinculados'))
        @foreach(old('vinculados') as $index => $vinculado)
            @if(!isset($responsavel->vinculados[$index]))
